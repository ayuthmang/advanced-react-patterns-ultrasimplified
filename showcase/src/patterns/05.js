...
const initialState = { count: 0, countTotal: 267, isClicked: false }
...
const useClapAnimation = ({ clapEl, countEl, clapTotalEl }) => {
  ...
  useLayoutEffect(() => {
    ...
    const triangleBrust = new mojs.Burst({
      parent: clapEl,
      radious: { 50: 95 },
      count: 5,
      angle: 30,
...
const MAXIMUM_USER_CLAP_VAL = 10

const Usage = () => {
  const [state, setState] = useState(INITIAL_STATE)
  const handleClap = () => {
    setState(({ count, countTotal }) => ({
      count: Math.min(count + 1, MAXIMUM_USER_CLAP_VAL),
      countTotal: count < MAXIMUM_USER_CLAP_VAL ? countTotal + 1 : countTotal,
      isClicked: true,
    }))
  }
